...
/**
 * A single uploaded invoice file and its audit outcome. `sessionId` is a plain
 * integer FK (no TypeORM relation decorator — same convention as the reference
 * project). The extracted text/OCR result is cached in `rawText` for debugging.
 */
@Entity({ name: 'invoices' })
@Index('idx_invoices_session', ['sessionId'])
@Index('idx_invoices_status', ['status'])
export class Invoice {
  @PrimaryGeneratedColumn('increment', { type: 'integer', primaryKeyConstraintName: 'pk_invoices' })
  id!: number
  ...
  @Column({ name: 'amount_in_total', type: 'boolean', nullable: false, default: false })
