...
const TITLE_CLEAN = /\/|纳税人识别号|统一社会信用|税\s*号|开票日期|密码区|规格型号|金额|税率|地址|电话|开户行|账号/

const TAX_ID_LABELED = /(?:纳税人识别号|统一社会信用代码|税号)[:：]?\s*([0-9A-HJ-NPQRTUWXY]{15,20})/
...
export function extractInvoiceFields(text: string): InvoiceFields {
  ...
  for (const pat of TITLE_PATTERNS) {
    ...
    if (m) {
      ...
      const full = (m[1]!.split(TITLE_CLEAN)[0] ?? '').trim().replace(/[\s:：\t]+$/, '')
