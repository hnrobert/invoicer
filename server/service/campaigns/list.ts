...
export async function campaignToPublic(c: Campaign): Promise<CampaignPublic> {
  return {
    id: c.id,
    userId: c.userId,
...
export async function listCampaignsFor(user: Pick<AuthUser, "id">): Promise<{
  ...
  for (const c of all) {
    (c.organizationId ? byOrg : personal).push(await campaignToPublic(c));
  }
