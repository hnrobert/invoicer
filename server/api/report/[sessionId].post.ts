...
/** Email a session's audit report (table + total) to a recipient. */
export default defineEventHandler(async (event) => {
  const sessionId = Number(getRouterParam(event, 'sessionId'))
  const { to } = await readBody<{ to?: string }>(event)
  if (!to) throw createError({ statusCode: 400, statusMessage: '请填写收件人邮箱' })
  ...
  const total = await calcTotal(sessionId)
  ...
  const bodyHtml = `
    ...
    <p style="margin:12px 0 0;color:#6b7280;font-size:12px;">抬头：${esc(session.expectedTitle)} · 税号：${esc(session.expectedTaxId)}</p>`
  ...
  const messageId = await sendMail({
    to,
    subject: `发票审核结果 · 合规金额 ¥${total.toFixed(2)}`,
    body: html,
    html: true,
  })
