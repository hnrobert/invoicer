...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ decision?: string; manual_amount?: number | string }>(event)

  if (body?.decision !== 'qualified' && body?.decision !== 'unqualified') {
    throw createError({ statusCode: 400, statusMessage: 'decision 必须为 qualified 或 unqualified' })
  }

  const repo = AppDataSource.getRepository(Invoice)
  const inv = await repo.findOneBy({ id: invoiceId, sessionId })
  if (!inv) throw createError({ statusCode: 404, statusMessage: '发票记录不存在' })

  let manualAmount = inv.manualAmount
  if (body.manual_amount != null && body.manual_amount !== '') {
    const n = Number(body.manual_amount)
    if (Number.isNaN(n)) throw createError({ statusCode: 400, statusMessage: '手动金额格式错误' })
    manualAmount = n
  }
  ...
  return { ok: true, record: invoiceToPublic(updated), total_amount: await calcTotal(sessionId) }
})
