...
export default defineEventHandler(async (event) => {
  ...
  if (["owner", "admin", "editor", "viewer", "member"].includes(name)) {
    throw createError({ statusCode: 400, statusMessage: "Cannot reuse a built-in role name" });
  }
  if (!(BASES as readonly string[]).includes(baseRole)) {
    throw createError({ statusCode: 400, statusMessage: "baseRole must be admin/editor/viewer/member" });
  }

  const repo = AppDataSource.getRepository(OrgCustomRole);
  const dup = await repo.findOneBy({ organizationId: orgId, name });
  if (dup) {
    throw createError({ statusCode: 400, statusMessage: "A role with this name already exists" });
  }
  await repo.save({
